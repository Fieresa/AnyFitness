...
";
	...
	function top(){
		$sql="select client,c.nom nom,prenom,sum(quantite*prix) top from commandes inner join produit p on p.id=produit inner join client c on c.id=client where paye=1 group by client order by top desc ";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste; 
	...
	function topmois($month,$year){
		$sql="select client,c.nom,prenom,sum(quantite*prix) top from commandes inner join produit p on p.id=produit inner join client c on c.id=client where (extract(month from date)-$month=0) and (extract(year from date)-$year=0) and paye=1 group by client order by top desc ";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste; 
