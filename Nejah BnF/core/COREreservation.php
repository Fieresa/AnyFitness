...
function afficherReservation ($reservation){
		...
		echo "dates: ".$reservation->getdates()."<br>";
	}
	function calculerSalaire($reservation){
		echo 10 * 10;
	}
	function ajouterreservation($reservation){
		$sql="insert into reservation (Id_reservation,Id_cour,Id_client,dates) values (:Id_reservation, :Id_cour,:Id_client,:dates)";
	...
	function modifierreservation($reservation,$Id_reservation){
		$sql="UPDATE reservation SET Id_reservation=:Id_reservation, Id_cour=:Id_cour,Id_client=:Id_client,dates=:dates WHERE Id_reservation=:Id_reservation";
...
try{		
        ...
        $Id_client=$reservation->getId_client();
        $dates=$reservation->getdates();
		$datas = array(':Id_reservation'=>$Id_reservation, ':Id_cour'=>$Id_cour, ':Id_client'=>$Id_client, ':dates'=>$dates);
		$req->bindValue(':Id_reservation',$Id_reservation);
		$req->bindValue(':Id_cour',$Id_cour);
		$req->bindValue(':Id_client',$Id_client);
		$req->bindValue(':dates',$dates);
  ...
  print_r($datas);
	...
	function rechercherListereservation($Id_cour){
		$sql="SELECT * from reservation where Id_cour=$Id_cour";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
