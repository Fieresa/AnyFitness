...
function affichercour ($cour){
	...
	function ajoutercour($cour){
		$sql="insert into cour (Id_coach,Id_cour,Capacity,Id_backupcoach,Duration,Heure,Jour) values (:Id_coach, :Id_cour,:Capacity,:Id_backupcoach,:Duration,:Heure,:Jour)";
	...
	function modifiercour($cour,$Id_cour){
		$sql="UPDATE cour SET Id_coach=:Id_coach, Id_cour=:Id_cour,Id_backupcoach=:Id_backupcoach,Capacity=:Capacity,Duration=:Duration,Heure=:Heure,Jour=:Jour WHERE Id_cour=:Id_cour";
...
try{		
  ...
  print_r($datas);
	...
	function rechercherListecour($Id_cour){
		$sql="SELECT * from cour where Id_coach=$Id_coach";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
