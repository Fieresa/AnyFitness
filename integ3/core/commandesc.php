...
function affichercommande ($commandes){
	...
	function affichercommandesp($str){
		//$sql="SElECT * From employe e inner join formationphp.employe a on e.cin= a.cin";
		$sql="SElECT numcmd,quantite,folder,nom,prix,date,heure,Qt From commandes inner join produit on produit=id where client=$str and paye=0 order by date desc,heure desc";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
